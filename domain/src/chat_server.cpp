...
namespace fast_chat {
    ...
    std::shared_ptr<ChatRoom> ChatServer::create_room(const std::string& name, const std::shared_ptr<IMessageNotifier> message_notifier) {
        assert(!name.empty());

        if (std::ranges::any_of(rooms, [&name](const std::shared_ptr<ChatRoom>& room) {
            return room->get_name() == name; })) {
